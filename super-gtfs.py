...
new_routes = pd.DataFrame(columns=['route_id', 'agency_id', 'route_type'])
...
for el in lst_gtfs:
    ...
    if 'agency_id' in tmp_routes.columns.tolist():
        #tmp_routes = tmp_routes[['route_id', 'agency_id', 'route_type']]
        tmp_routes = tmp_routes[['route_id', 'agency_id', 'route_short_name', 'route_long_name', 'route_type']]
    else:
        #tmp_routes = tmp_routes[['route_id', 'route_type']]
...
#new_stop_times_a = pd.merge(left=new_stop_times, right=slownik, left_on='stop_id', right_on='stop_id', how='left')
